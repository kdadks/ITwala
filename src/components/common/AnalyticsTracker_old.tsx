...
const getCountryFromIP = async (): Promise<string> => {
  ...
  const services = [
    {
      name: 'ipify',
      url: 'https://geo.ipify.org/api/v1?apiKey=',
      fallback: true
    },
    {
      name: 'ipapi',
  ...
  // First try browser geolocation API if available
  try {
    if ('geolocation' in navigator) {
      // This is just for getting timezone, not exact location
    ...
    const trackPageView = async () => {
      try {
        ...
        try {
          ...
          if (selectError) {
            console.warn('Error checking existing session (table might not exist):', selectError);
          ...
          // Don't throw error - continue page tracking even if session tracking fails
        }
