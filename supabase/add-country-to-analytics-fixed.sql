-- Step 1: Add country column to page_views table if it doesn't exist
ALTER TABLE page_views 
ADD COLUMN IF NOT EXISTS country VARCHAR(100) DEFAULT 'Unknown';

-- Step 2: Add countries column to analytics_data table for aggregated country data
ALTER TABLE analytics_data
ADD COLUMN IF NOT EXISTS countries JSONB DEFAULT '[]';

-- Step 3: Update the aggregate_daily_analytics function to include country data
...
-- Step 4: Run aggregation for today to test (with correct date casting)
SELECT aggregate_daily_analytics(CURRENT_DATE::DATE);
SELECT aggregate_daily_analytics((CURRENT_DATE - INTERVAL '1 day')::DATE);

-- Step 5: Check if columns were added
SELECT 
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='page_views' AND column_name='country') as page_views_has_country,
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='analytics_data' AND column_name='countries') as analytics_data_has_countries;